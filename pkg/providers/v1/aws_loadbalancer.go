...
// or from AWS defaults. Only supported attributes by controller are reconciled.
//
// Parameters:
...
func (c *Cloud) reconcileTargetGroupsAttributes(ctx context.Context, lbARN string, annotations map[string]string) error {
	...
	for _, tg := range describeTargetGroupsOutput.TargetGroups {
		...
		if err != nil {
			errors = append(errors, fmt.Errorf("error updating target group attributes for target group %q: %w", aws.ToString(tg.TargetGroupArn), err))
		}
	}
	if len(errors) > 0 {
		return fmt.Errorf("one or more errors occurred while updating target group attributes: %v", errors)
	}
	return nil
}

// ensureTargetGroupAttributes ensures that the target group attributes for a specific
// target group match the desired state specified in service annotations or from AWS defaults.
//
// Parameters:
...
// based on the Service annotation, target group AWS defaults, and current attribute values, calculating
// only the attributes to be changed considering the following order of precedence:
// 1. User-specified attributes from ServiceAnnotationLoadBalancerTargetGroupAttributes annotation
// 2. AWS defaults
// 3. Current attribute values
//
...
// - Target groups calculate drift from current AWS default attributes values, and mark them as desired to restore.
//
// Parameters:
...
func (c *Cloud) buildTargetGroupAttributes(tg *elbv2types.TargetGroup, tgAttributes []elbv2types.TargetGroupAttribute, annotations map[string]string) ([]elbv2types.TargetGroupAttribute, error) {
	...
	// desiredAttributes are the AWS default values for a target group.
	// https://pkg.go.dev/github.com/aws/aws-sdk-go-v2/service/elasticloadbalancingv2@main/types#TargetGroupAttribute
