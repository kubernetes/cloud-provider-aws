...
type CreateTagsBatcher struct {
	batcher *batcher2.Batcher[ec2.CreateTagsInput, ec2.CreateTagsOutput]
}
...
func NewCreateTagsBatcher(ctx context.Context, ec2api iface.EC2) *CreateTagsBatcher {
	options := batcher2.Options[ec2.CreateTagsInput, ec2.CreateTagsOutput]{
		Name:          "create_tags",
	...
	return &CreateTagsBatcher{batcher: batcher2.NewBatcher(ctx, options)}
}
...
func execCreateTagsBatch(ec2api iface.EC2) batcher2.BatchExecutor[ec2.CreateTagsInput, ec2.CreateTagsOutput] {
	return func(ctx context.Context, inputs []*ec2.CreateTagsInput) []batcher2.Result[ec2.CreateTagsOutput] {
		results := make([]batcher2.Result[ec2.CreateTagsOutput], len(inputs))
		firstInput := inputs[0]
		...
		if err != nil {
			...
			for idx, input := range inputs {
				...
				go func(input *ec2.CreateTagsInput) {
					...
					results[idx] = batcher2.Result[ec2.CreateTagsOutput]{Output: out, Err: err}
			...
			for idx := range inputs {
				results[idx] = batcher2.Result[ec2.CreateTagsOutput]{Output: output}
			}
		}
		return results
	}
