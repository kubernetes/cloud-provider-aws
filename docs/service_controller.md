...
| service.beta.kubernetes.io/aws-load-balancer-security-groups                   | Comma-separated list                | -   | ELB,NLB | Specifies the security groups to be added to ELB or NLB. Differently from the annotation "service.beta.kubernetes.io/aws-load-balancer-extra-security-groups", this replaces all other security groups previously assigned to the ELB. For NLBs the annotation requires `NLBSecurityGroupMode = Managed` to be set in the cloud-config. |
